...
The conversation core runs in the main process. A conversation is a scene: the main process derives each session's `scene_id` from this profile's Desktop identity and routes every streamed event and history row back to its own session by that key, so several sessions can be in flight at once. The renderer holds no token and makes no request of its own — `tests/architecture.test.ts` fails if `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource` or `sendBeacon` appears anywhere in its source; it subscribes to the `beings:chat-*` IPC channels and renders rows the main process has already reconciled. Links a Being writes in a message are shown as text with the address in their tooltip; nothing in a message opens a website by itself. The sandboxed `beings://chat` page and its request proxy were removed on 2026-09-16; see [MIGRATION.md](MIGRATION.md).
