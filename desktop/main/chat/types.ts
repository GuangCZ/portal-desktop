// The chat layer's types: the wire shapes of the five Being message endpoints,
// and the durable transcript shapes behind them. Ported from BeingDesktop 0.8.26
// src/being-chat.cjs (509 lines), src/being-recovery.cjs (461 lines),
// src/chat-store.cjs (290 lines), src/session-titles.cjs, src/session-recovery.cjs
// and docs/interfaces.md §7「持久化格式」; 2026-09-16. Merged from the parallel
// stages' protocol-types.ts and store-types.ts on the same date.
//
// Every wire shape here is what a real Being was measured to send, not what its
// source suggests it might — see docs/desktop-message-layer.md (that file's §一
// through §十 are cited by section number throughout this port).
//
// Wire and disk meet at exactly one place: `HistoryRow` is what `/api/history`
// returns and what `ChatStore.apply` folds into `StoredRow`s, so the two halves
// of this file share it rather than each declaring their own.
...
// ---------------------------------------------------------------------------
// Durable transcript: what the encrypted per-Being cache file holds, and the
// projections the session layer builds from it.
// ---------------------------------------------------------------------------
...
/** One page of `/api/history` on its way into the store. It is deliberately not
 * `HistoryPage` above: the wire page is already validated, while this is whatever
 * a caller passes, so every field is re-checked by `storedRow`. `baseline` marks a
 * full load rather than an increment, which is what lifts the seeded gate
 * (invariant 5). */
