...
客户端托盘提供恢复窗口和退出入口；Dock 激活与第二次启动同样恢复原窗口。`client-startup.ts` 独立管理 macOS/Windows 的客户端登录自启，读取系统状态并校验写入结果，不依赖 Being 配置或 Portal 生命周期。Windows NSIS 和便携版使用稳定目录中的当前可执行文件，旧 Squirrel 安装仍识别其稳定启动器；开发模式与 Linux 不注册登录项。
