...
try {
  $running = @(Get-CimInstance Win32_Process -Filter "Name='being-desktop.exe'" | Where-Object {
  ...
  do {
    $remaining = @(Get-CimInstance Win32_Process -Filter "Name='being-desktop.exe'" | Where-Object { $_.ExecutablePath -eq $Executable })
