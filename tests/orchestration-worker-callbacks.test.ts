...
/* src/security.cjs `parseConnection` / `sessionPartition`, copied verbatim: the connection unit
   owns them, and this unit injects them into the two senders. */
const SESSION_IDENTITY_VERSION = "v1";
function parseConnection(input: string): LoomConnection {
  ...
  return {
    url: url.href, apiBase: api.href.replace(/\/+$/, ""), token, secret,
    displayUrl: `${url.origin}${url.pathname}`, beingName: decodeURIComponent(url.pathname.split("/").filter(Boolean).pop() || "Being"),
  };
}
function sessionPartition(connection: LoomConnection): string {
