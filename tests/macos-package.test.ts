...
it.skipIf(process.platform !== 'darwin' || process.env.PORTAL_DESKTOP_MAC_PACKAGE_TESTS !== '1')(
  'installs the signed DMG and stages its matching Release ZIP without launching another client', async () => {
    ...
    let mounted = false;
    try {
      const current = path.join(root, 'Applications/Being Desktop.app');
      ...
      await expect(macInstallLocation(path.join(source, 'Contents/MacOS/Being Desktop'))).rejects.toThrow('不可写');
      ...
      const handoff = await stageInstaller(path.join(root, 'profile'), version, 'd5z/portal-desktop', path.join(current, 'Contents/MacOS/Being Desktop'),
